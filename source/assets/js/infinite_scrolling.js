...
// Shamelessly copied and pretified from
// http://static.tumblr.com/c7cvhvu/mCZmb3z94/is.js
        ...
        function addNextPage(oj) {
            ...
            for (var i = 0; i < posts.length; i++) {
                ...
                if (child.nodeName.toLowerCase() == "script") {
                    child = cloneScriptNode(child);
                } else if (child.nodeType === Node.ELEMENT_NODE) {
                    scripts = child.querySelectorAll('script');
                    for (j = 0; j < scripts.length; j++) {
                        cloneScriptNode( scripts[j], true );
                    }
                }
                tAP.pp.appendChild(child);
            }
            var footer = $("footer");
            footer ? footer.parentNode.appendChild(footer) : null;
            tAP.rF = true;
        }
        function cloneScriptNode(original, replace){
            var clone = document.createElement("script");
            clone.type  = "text/javascript";
            if (original.src)  clone.src  = original.src;
            if (original.text) clone.text = original.text;
            if (replace === true && original.parentElement) clone = original.parentElement.replaceChild(clone, original);
            return clone;
        }
        watch_scroll();
